...
* every node may carry an arbitrary `metadata` dict (used by `regulatory_tag_overlay`; see the [API reference](../api.md#plotting)).
...
| Key | Used by | Meaning |
|-----|---------|---------|
| `tag` | `regulatory_tag_overlay` (see the [API reference](../api.md#plotting)) | A categorical label such as `"PII"`, `"financial"`, `"behavioural"`. |
