...
def test_concept_disparity_long_form_columns(graph, shap_arr, protected_series) -> None:
    names, arr = shap_arr
    ...
    n_groups = 3
    assert len(df) == n_groups * len(graph)
    assert df.attrs["agg"] == "mean_abs"
    assert df.attrs["reference_group"] == "A"
    assert df.attrs["protected_order"] == ["A", "B", "C"]


def test_concept_disparity_reference_row_is_zero(graph, shap_arr, protected_series) -> None:
    names, arr = shap_arr
    df = concept_disparity(graph, names, arr, protected_series, reference="A")
...
def test_concept_disparity_string_form_requires_X(graph, shap_arr, protected_series) -> None:
    names, arr = shap_arr
    X = pd.DataFrame(arr, columns=names).assign(cohort=protected_series.to_numpy())
    df = concept_disparity(graph, names, arr, "cohort", X=X, reference="A")
...
def test_concept_disparity_rejects_bad_input(graph, shap_arr, protected_series) -> None:
    names, arr = shap_arr
    with pytest.raises(ValueError, match="2D"):
        concept_disparity(graph, names, arr.sum(axis=1), protected_series, reference="A")
...
def test_concept_disparity_heatmap_renders(graph, shap_arr, protected_series) -> None:
    names, arr = shap_arr
    df = concept_disparity(graph, names, arr, protected_series, reference="A")
    fig = concept_disparity_heatmap(graph, df)
    assert fig.data
...
def test_concept_disparity_heatmap_drops_reference_with_toggle(
    ...
    names, arr = shap_arr
    df = concept_disparity(graph, names, arr, protected_series, reference="A")
    fig = concept_disparity_heatmap(graph, df, include_reference=False)
...
def test_concept_disparity_heatmap_rejects_missing_columns(graph) -> None:
    bad_df = pd.DataFrame({"name": ["a"], "kind": ["concept"], "value": [1.0]})
    with pytest.raises(KeyError, match="protected_group"):
        concept_disparity_heatmap(graph, bad_df)
