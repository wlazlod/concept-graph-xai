...
def test_concept_sankey_renders(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr)
    assert fig.data
    assert fig.data[0].type == "sankey"


def test_concept_sankey_node_layout_two_level_graph(simple_graph, shap_arr) -> None:
    # Two-level simple_graph: features connect directly to top-level concepts.
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr)
...
def test_concept_sankey_includes_intermediate_concepts() -> None:
    # Three-level simple_graph: features → sub-concept → top-level concept.
...
def test_concept_sankey_link_count_matches_hierarchy(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr)
...
def test_concept_sankey_max_features_per_concept_caps(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr, max_features_per_concept=1)
    labels = list(fig.data[0].node.label)
    # 1 feature per concept * 2 concepts = 2 features + 2 concepts + 2 outcomes = 6
    assert len(labels) == 6


def test_concept_sankey_rejects_bad_input(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    with pytest.raises(ValueError, match="2D"):
        concept_sankey(simple_graph, names, arr.sum(axis=1))
    with pytest.raises(ValueError, match="cols"):
        concept_sankey(simple_graph, names[:2], arr)


def test_concept_sankey_link_values_are_nonnegative(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr)
...
def test_concept_sankey_outcome_split_matches_signed_flow(simple_graph, shap_arr) -> None:
    names, arr = shap_arr
    fig = concept_sankey(simple_graph, names, arr)
    labels = list(fig.data[0].node.label)
    sources = list(fig.data[0].link.source)
...
def test_concept_sankey_subconcept_outflow_lte_inflow_under_cancellation() -> None:
    ...
    simple_graph = ConceptGraph.from_dict({"Risk": {"Behaviour": {"Delinquency": ["d1", "d2"]}}})
    names = ["d1", "d2"]
...
def test_concept_sankey_skips_features_directly_under_root() -> None:
    ...
    simple_graph = ConceptGraph.from_dict({"Risk": ["a", "b"]})
    arr = np.random.default_rng(0).standard_normal((10, 2))
    with pytest.raises(ValueError, match="non-zero SHAP magnitude under a concept"):
        concept_sankey(simple_graph, ["a", "b"], arr)
