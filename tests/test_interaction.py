...
def test_interaction_matrix_is_square_and_symmetric(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr)
    assert df.shape[0] == df.shape[1]
    assert list(df.index) == list(df.columns)
    np.testing.assert_allclose(df.to_numpy(), df.to_numpy().T, atol=1e-12)


def test_interaction_matrix_drops_root_by_default(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr)
    assert simple_graph.root not in df.index
    assert {"Income", "Behaviour"}.issubset(set(df.index))


def test_interaction_matrix_only_concepts_excludes_features(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr, only_concepts=True)
    assert "x1" not in df.index
    df_with_features = concept_interaction_matrix(simple_graph, names, arr, only_concepts=False)
    assert "x1" in df_with_features.index


def test_interaction_matrix_diagonal_equals_self_block_sum(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr, agg="mean_abs")
...
def test_interaction_matrix_signed_can_be_negative(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr, agg="mean_signed")
    assert df.attrs["agg"] == "mean_signed"
    # The signed agg should give a different number from mean_abs for at least one cell
    df_abs = concept_interaction_matrix(simple_graph, names, arr, agg="mean_abs")
    assert not np.allclose(df.to_numpy(), df_abs.to_numpy())


def test_interaction_matrix_rejects_bad_input(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    with pytest.raises(ValueError, match="3D"):
        concept_interaction_matrix(simple_graph, names, arr.sum(axis=0))
    with pytest.raises(ValueError, match="square"):
        concept_interaction_matrix(simple_graph, names, arr[:, :, :3])
    with pytest.raises(ValueError, match="features"):
        concept_interaction_matrix(simple_graph, ["x1", "x2"], arr)


def test_interaction_heatmap_renders(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr)
...
def test_interaction_heatmap_signed_uses_diverging_palette(simple_graph, interaction_arr) -> None:
    names, arr = interaction_arr
    df = concept_interaction_matrix(simple_graph, names, arr, agg="mean_signed")
    fig = concept_interaction_heatmap(df)
...
def test_interaction_heatmap_annotations_capped() -> None:
    # Larger simple_graph so we have several off-diagonal cells
